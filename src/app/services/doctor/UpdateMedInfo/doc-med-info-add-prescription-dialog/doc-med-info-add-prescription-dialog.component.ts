...
export class DocMedInfoAddPrescriptionDialogComponent {
  ...
  constructor(private dialog: MatDialogRef<DocMedInfoAddPrescriptionDialogComponent>, public doc: DoctorService, public router: Router, private snackbar: MatSnackBar
  ) {
  ...
  onSubmit(f: any) {
    ...
    this.doc.AddPriscription(myPres).subscribe(response => {
      ...
      this.snackbar.open('Prescription Added Successfully', 'Ok', {
        duration: 3000,
      })
