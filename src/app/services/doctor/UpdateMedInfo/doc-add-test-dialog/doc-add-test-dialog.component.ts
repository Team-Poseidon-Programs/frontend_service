...
export class DocAddTestDialogComponent {
  constructor(private dialog: MatDialogRef<DocAddTestDialogComponent>, public doc: DoctorService, public router: Router, private snackbar: MatSnackBar
  ) {
  ...
  onSubmit(f: any) {
    ...
    this.doc.AddTest(myTest).subscribe(response => {
      ...
      this.snackbar.open('New test added successfully', 'Ok', {
        duration: 3000,
      });
